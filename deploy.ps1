...
param(
    ...
    [ValidateSet("left", "right", "right_bare", "left_bare", "left_nostudio", "settings_reset")]
    [string]$Target = "left",
...
function Find-Uf2([string]$target, [string]$dir) {
    ...
    $filter = switch ($target) {
        "left" { { $_.Name -like "corne_v3_left*.uf2" -and $_.Name -notlike "corne_v3_left_bare*" -and $_.Name -notlike "corne_v3_left_nostudio*" } }
        "right" { { $_.Name -like "corne_v3_right*.uf2" -and $_.Name -notlike "corne_v3_right_bare*" } }
        "left_bare" { { $_.Name -like "corne_v3_left_bare*.uf2" } }
        "left_nostudio" { { $_.Name -like "corne_v3_left_nostudio*.uf2" } }

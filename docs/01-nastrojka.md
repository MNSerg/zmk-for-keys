...
1. Прошить **обе** стороны из **одного** zip одной сборки (не left без right и наоборот).
2. Если не видят друг друга — `settings_reset` на **обе**, затем снова left+right.
3. Правая должна быть под питанием (USB или батарея).

Правая **не** появляется в списке Bluetooth ПК — это нормально.

После включения pointing (мышь в HID): на ПК **удалите** старое сопряжение Corne и сделайте новое, иначе курсор по BLE может не идти (USB при этом часто работает).
